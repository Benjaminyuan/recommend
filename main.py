...
def train():
    ...
    for epoch in range(args.epoch):
        ...
        for idx in range(n_batch):
            # 采样, 用户id，正样本（存在的边），负样本（不存在的边）
            users, pos_items, neg_items = data_generator.sample()
            # model(data) == model.forward()
            u_g_embeddings, pos_i_g_embeddings, neg_i_g_embeddings = model(users,
                                                                           pos_items,
                                                                           neg_items,
                                                                           drop_flag=args.node_dropout_flag)
            # 计算损失
            batch_loss, batch_mf_loss, batch_emb_loss = model.create_bpr_loss(u_g_embeddings, pos_i_g_embeddings,
                                                                              neg_i_g_embeddings)
            optimizer.zero_grad()
            batch_loss.backward()
            optimizer.step()
