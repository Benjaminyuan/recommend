...
class NGCF(nn.Module):
    ...
    def _convert_sp_mat_to_sp_tensor(self, X):
        """把scipy的稀疏矩阵转化为pytorch的稀疏矩阵

        """
        # 转化成 coo形式的压缩矩阵
        coo = X.tocoo()
        # idx 列表[[row idx],[col idx]]
    ...
    def forward(self, users, pos_items, neg_items, drop_flag=True):
        ...
        # 拼接user和item的嵌入，用于计算,表示 l-1 层的嵌入表示
        ...
        for k in range(len(self.layers)):
            ...
            # bi messages of neighbors. 邻居的信息
            ...
            # E_l = (L*E) * W_c_k + B_c_k + E * L * E * W_b_k + b
    ...
    def create_bpr_loss(self, users, pos_items, neg_items):
        """
        users: 用户的嵌入 len(users) == batch_size
        pos_items: 正样本的嵌入
        neg_items: 负样本的嵌入
        """
